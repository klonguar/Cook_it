class ApplicationController < ActionController::Base
  ...
  protect_from_forgery with: :exception
    ...
    helper_method :current_user, :logged_in? #these helper methods ara available to other controller by default but they are not availbele for the view, 
                                             #so with this statement, rails will make the following methods available to the view
    def current_user
      @current_user ||= User.find(session[:user_id]) if session[:user_id] #return this user if session[:user_id] that is stored in our session hash if so then find the user in our database and display it, 
